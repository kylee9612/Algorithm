...
class Solution {
	...
	//	새로배운 유클리드 호제법
	//	서로를 나누어 떨어지기 전까지 반복한다
	//	w % h == 0 이 될때까지
	//	큰 수에 h
	//	작은수에 w % h인 나머지
	private long getGCDUclid(long w, long h) {
		if(h == 0) return w;
		return getGCDUclid(h, h%w);
	}
}

public class main {
	public static void main(String[] args) {
		new Solution().solution(10, 8);
	}
}
